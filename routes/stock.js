...
stockRoutes.route("/stock").get(function (req, res) {
  let db_connect = dbo.getDb("stocks");
  db_connect
...
stockRoutes.route("/stock/add").post(function (req, response) {
  ...
  db_connect.collection("stocks_data").insertOne(myobj, function (err, res) {
    if (err) throw err;
    response.json(res);
  });
});
...
stockRoutes.route("/:id").delete((req, response) => {
  let db_connect = dbo.getDb();
  let myquery = { _id: ObjectId( req.params.id )};
  db_connect.collection("stocks_data").deleteOne(myquery, function (err, obj) {
    if (err) throw err;
